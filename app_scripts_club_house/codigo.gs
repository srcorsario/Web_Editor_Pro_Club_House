// REPOSITORIO: google-apps-script
// ARCHIVO: Código.gs (Versión con LockService — evita condición de carrera
//          que borraba las columnas INFO_* cuando dos guardados se solapaban)
//
...
//
// NUEVO (25 agosto): paridad con US Open — se añade la columna base "Opciones_Inactivas"
// (33ª columna) para que el modo "Plato con ingredientes" del Web Editor Pro también oculte
// en la web pública de RG los ingredientes/opciones que se desactiven (antes solo funcionaba
// en US Open). No hace falta tocar la hoja a mano: en cuanto se implemente esta versión, el
// primer "Guardar cambios" del editor la crea sola con las cabeceras correctas, igual que ya
// pasa con INFO_* y con la hoja "Categorias". CÓMO DESPLEGARLO: Extensiones > Apps Script >
// sustituir todo el contenido de Código.gs por el de este archivo > Implementar > Gestionar
// implementaciones > icono de lápiz sobre la implementación web existente > Versión: Nueva
// versión > Implementar. OJO: hay que EDITAR la implementación existente (no crear una
// nueva), para que la URL ?exec no cambie.
// =========================================
...
function servirCsvEnVivo(idiomasParam, idsParam, soloBaseParam) {
  ...
  // NUEVO: se añade OPCIONES_INACTIVAS a las columnas base (paridad con US Open) — guarda,
  // por plato, las posiciones de las opciones/ingredientes desactivados entre "//.../ /" del
  // Nombre_ES (ver Web Editor Pro > modo "Plato con ingredientes"). Es la misma para todos
  // los idiomas, así que va como columna base igual que Alergenos_Cod, no por idioma.
...
function doPost(e) {
  ...
  try {
    ...
    // depende de la traducción), así que vive como una columna base más, igual que
    // Alergenos_Cod — ver la función auxiliar "extraerOpciones"/"opcionesActivasTexto" del
    // lado de la web pública para cómo se aplica al render.
    // NUEVO (26 agosto): se añade "Hash_Fila" (última columna) — hash de detección de cambios
    // de TODO el contenido de la fila (todas las columnas menos ella misma), recalculado más
    // abajo en cada guardado (ver calcularHashFila). No lo escribe el editor ni viene en el
    // payload: este propio script lo calcula y sobreescribe siempre con el valor real, así que
...
// del menú (p.ej. "Niños") para que no aparezca en la web pública, sin tocar el Activa de
...
// POST ?accion=categorias — body JSON: { "id": "7", "activa": true|false }.
